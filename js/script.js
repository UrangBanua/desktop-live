...
function showForecastWeather(data){
	...
	for(var i in daysIndex){
		...
		if(values["temp"][daysIndex[i]]!=undefined){
			...
			tempString=leadingZero(Math.round(minT))+'&deg;/'+leadingZero(Math.round(maxT))+'&deg;'
		}
		else {
			tempString="NoData";
		}
		...
		document.getElementById(`forecasttemp${Math.floor(i)+1}`).innerHTML=tempString;
	...
	for(var z=1;z<dd.length;z++)
	{
		if(dd[z]!=true)
		{
			document.getElementById('forecastday'+z).style.display="none";
			document.getElementById('forecasttemp'+z).style.display="none";
			document.getElementById('forecastimg'+z).style.display="none";
		}
		else
		{
			document.getElementById('forecastday'+z).style.display="block";
			document.getElementById('forecasttemp'+z).style.display="block";
			document.getElementById('forecastimg'+z).style.display="block";
		}
	}
